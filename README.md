...
- 在邮件详情中使用不带人格的 LLM 总结或翻译，每类任务只保留最新结果；
...
同一个私聊用户可以绑定多个账户；每个账户只能绑定一个私聊用户。管理员 UID 可以在私聊中管理全部账户，普通用户只能在绑定的平台中操作绑定给自己的账户。
...
邮件详情的“总结邮件”和“翻译邮件”不会加载当前聊天人格。可在“邮件中心 WebUI”配置区指定模型；留空时使用该邮箱绑定私聊当前生效的模型。翻译默认目标语言留空时跟随 AstrBot WebUI 的界面语言。每封邮件的总结和翻译各只保留一份最新缓存；正文没有变化时再次点击按钮或自动显示时直接返回最新结果，不按当前默认语言筛选。
...
配置页面按板块进行分组。后端只读取当前分组结构，`mail_accounts` 保持为顶层账户列表。
...
邮件中心会合并同一账户、文件夹和 UID 的并发正文请求；已有正文缓存且校验通过时，会在“邮件中心 WebUI”的“邮件云端校验冷却”时间内复用结果，默认 5 分钟。频繁在几封邮件之间切换时仍会立即显示本地正文，但不会在冷却期内反复建立相同的 IMAP 校验请求。发现变化或删除时不复用“正常”结果，仍按原有逻辑及时标记。总结/翻译缓存始终只读取本地 SQLite。
...
总结和翻译结果仍使用邮件内容哈希以及提示词版本校验有效性，但同一封邮件的每种任务只保留最新一次生成结果。重新指定语言生成会替换旧语言结果；确认邮件删除、移动、UIDVALIDITY 变化、邮件头变化或读取正文时发现内容变化后，对应缓存会被清除，仅网络失败不会清理缓存。

...
邮件中心会合并同一账户、文件夹和 UID 的并发正文请求；已有正文缓存且校验通过时，结果会复用 30 秒。频繁在几封邮件之间切换时仍会立即显示本地正文，但不会在冷却期内反复建立相同的 IMAP 校验请求。发现变化或删除时不复用“正常”结果，仍按原有逻辑及时标记。总结/翻译缓存始终只读取本地 SQLite。

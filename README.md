...
- 在邮件详情或 Bot 私聊工具中使用不带人格的 LLM 总结或翻译，每类任务只保留最新结果；
...
邮件详情和 Bot 工具的“总结邮件”“翻译邮件”不会加载当前聊天人格。可在“邮件总结与翻译”配置区指定模型；留空时使用该邮箱绑定私聊当前生效的模型。默认目标语言留空时，WebUI 跟随界面语言，Bot 工具使用当前对话语言。每封邮件的总结和翻译各只保留一份最新缓存；未明确指定目标语言时直接返回最新结果，不按当前默认语言筛选。

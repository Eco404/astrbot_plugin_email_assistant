...
- 使用 `plugin_data` SQLite 本地邮件索引，支持新增邮件同步、渐进式全历史回填、云端删除核对和跨文件夹缓存复用；
...
- 邮件中心支持桌面双栏工作台和手机端抽屉导航、单邮件阅读返回及触屏确认操作；
...
数据库 schema 当前为 `v1`，版本记录保存在 `db_version` 表中。由于插件尚未正式发布，本次不迁移此前未带版本表的开发数据库；检测到旧库时会停用本地索引并在日志中提示删除 `mail_headers.db`，重新启动后会按云端数据建立新库。后续 schema 变更将以该版本记录为迁移依据。

本地索引将稳定邮件实体和 IMAP 位置分开保存。邮件实体优先使用 Gmail `X-GM-MSGID`，其他服务器使用规范化 `Message-ID`，两者均不可用时才使用邮件头指纹；文件夹、UIDVALIDITY 和 UID 只表示该实体当前所在的位置。正文、总结和翻译缓存关联邮件实体，不再绑定单个文件夹 UID。因此同一账户内的邮件移动或复制到其他已索引文件夹后，可以继续使用原缓存。
...
- **云端已删除或移动邮件，本地暂时不知道**：在下一次定期核对前，该邮件可能仍出现在本地列表中。WebUI 若有正文缓存会先显示缓存再后台校验，并把当前标题标红；刷新或下次进入页面后隐藏失效位置。Bot 查看、读取最新邮件或回复仍会先完成实时校验；若当前位置不存在，会把本地位置标记为 `remote_missing` 并拒绝以旧 UID 操作。
- **邮件移动到其他文件夹**：旧位置失效后，因位置失效触发的清理会宽限 24 小时。目标文件夹在此期间同步到相同 `X-GM-MSGID` 或 `Message-ID` 后，会把新 UID 关联到同一实体并继续使用正文、总结和翻译缓存。目标文件夹尚未建立索引时，仍需等待该文件夹的下一轮同步，不能继续使用旧 UID；正文仍可能受总容量和保留期限策略提前淘汰。
- **邮件确实从云端删除**：若宽限期后仍找不到该实体的任何有效位置，开启“云端删除时清理正文缓存”时会在后续云端核对中清理正文；总结和翻译缓存也会清理。关闭正文清理时，正文仍受缓存保留天数和总容量限制。
...
- **文件夹 UIDVALIDITY 变化**：说明服务器重建了文件夹 UID 空间，旧数字 UID 不能再信任。插件会把旧一代位置整体标记为失效、用新 UIDVALIDITY 建立索引，并把新邮件通知游标重置到当前基线，不推送重建前的历史邮件。新位置若能识别为同一邮件实体，仍可复用已有缓存。
...
- `body_cache_purge_on_remote_delete`：确认邮件没有其他有效云端位置后清理正文，默认开启；失效位置有 24 小时宽限期，以便目标文件夹同步并接管缓存。

正文缓存以 `account_id + 稳定邮件实体键` 保存，通过各文件夹的 UID 位置访问。普通网络错误不会被误判为云端删除。缓存只是阅读与未来搜索的副本，查看、回复和其他外部操作仍以实时云端校验为准。

总结和翻译结果仍使用邮件内容哈希以及提示词版本校验有效性，但同一封邮件的每种任务只保留最新一次生成结果。重新指定语言生成会替换旧语言结果；邮件移动或 UIDVALIDITY 变化且重新识别为同一实体时保留缓存，确认实体已删除、邮件头内容变化或读取正文时发现内容变化后清除对应缓存，仅网络失败不会清理缓存。
...
- 在手机端可通过顶部菜单打开账户、文件夹和草稿导航；打开邮件后进入单焦点详情视图，可随时返回列表。

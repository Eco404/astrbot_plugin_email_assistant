...
本地邮件头保存账户 ID、文件夹、UIDVALIDITY、UID、主题、发件人、日期、回复地址、线程头和附件存在状态，不保存附件、邮箱密码或代理密码。列表查询优先使用本地索引；查看详情、读取最新一封正文和回复邮件时仍会实时连接 IMAP 验证并读取云端原件。正文是否在读取后缓存由下述正文缓存配置决定。
...
`/email sync [account_id]` 会立即执行一轮新增同步和历史回填，并强制核对当前文件夹的云端 UID 集合。`/email status` 会显示有效索引数、云端失效数、历史回填状态和最近同步时间。后台轮询也会自动维护索引；即使账户关闭接收通知，只要查询功能开启，仍会同步邮件头。历史邮箱较大时不会在一次同步中全部拉完，以避免长时间阻塞、IMAP 限流和代理链路超时。
...
当前版本只建立草稿存储和并发控制基础，不注册 LLM 写信工具、不自动发送草稿，也不把草稿同步到邮箱服务商的 Drafts 文件夹。后续 WebUI 和 LLM 发信阶段应在此数据层上增加权限校验、人工确认和 SMTP/IMAP Drafts 同步。
